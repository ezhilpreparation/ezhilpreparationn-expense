...
export default function TagMergeModal({ 
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-3 sm:p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-sm sm:max-w-md max-h-[90vh] sm:max-h-[85vh] flex flex-col">
        <div className="flex items-center justify-between p-4 sm:p-6 border-b">
          <h2 className="text-lg sm:text-xl font-semibold text-gray-900">Merge Tag</h2>
          <button
            onClick={onClose}
        ...
        <div className="p-3 sm:p-4 lg:p-6">
          <div className="mb-6">
            <h3 className="text-sm sm:text-base font-medium text-gray-700 mb-3">Merge Operation</h3>
            <div className="flex items-center space-x-3 p-3 bg-gray-50 rounded-lg">
              <div className="flex items-center space-x-2">
                <span className="px-2 py-1 bg-red-100 text-red-700 rounded text-sm font-medium">
                  {sourceTag.tag.name}
                </span>
                <span className="text-xs sm:text-sm text-gray-500">({sourceTag.transactions})</span>
              </div>
              ...
              <div className="flex items-center space-x-2">
                {selectedTag ? (
                  <>
                    ...
                    <span className="text-xs sm:text-sm text-gray-500">({selectedTag.transactions})</span>
                  </>
                ) : (
                  <span className="text-xs sm:text-sm text-gray-400">Select target</span>
                )}
              </div>
            </div>
          ...
          <div className="max-h-40 sm:max-h-48 overflow-y-auto">
              ...
              {filteredTags.map((tag) => (
                ...
                >
                  <div className="flex items-center space-x-2 sm:space-x-3">
                    <span className="text-sm sm:text-base font-medium text-gray-900 truncate">{tag.tag.name}</span>
                    <span className="text-xs sm:text-sm text-gray-500">({tag.transactions})</span>
                  </div>
                  {selectedTag?.tag.id === tag.tag.id && (
            ...
            {filteredTags.length === 0 && (
              <div className="text-center py-6 sm:py-8 text-xs sm:text-sm text-gray-500">
                {searchTerm ? `No tags found matching "${searchTerm}"` : 'No other tags available'}
              </div>
            )}
          </div>
        </div>
        ...
        <div className="p-3 sm:p-4 lg:p-6 border-t">
          <div className="flex gap-2 sm:gap-3">
            ...
            <button
              ...
              className="flex-1 px-3 sm:px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700 transition-colors text-sm sm:text-base font-medium disabled:opacity-50 disabled:cursor-not-allowed"
            >
