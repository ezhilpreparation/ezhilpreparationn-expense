...
function TransactionForm() {
  ...
  useEffect(() => {
    if (isEditing && transaction) {
      ...
      // txnDate already in format "YYYY-MM-DD"
      setValue('date', transaction.txnDate);
      // txnTime like "HH:mm:ss" → trim seconds if your TimePicker expects only "HH:mm"
      const [hours, minutes] = transaction.txnTime.split(':');
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      const txnDate = data.date; // already "YYYY-MM-DD"
      const txnTime = `${hours.toString().padStart(2, "0")}:${minutes.toString().padStart(2, "0")}:00`;
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        {selectedAccount.linkedPaymentModes.map((paymentMode) => (
                          <button
                            ...
                            className={`p-2 text-left border rounded-lg transition-colors ${watchedValues.fromPaymentModeId === paymentMode.id
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        {selectedToAccount.linkedPaymentModes.map((paymentMode) => (
                          <button
                            ...
                            className={`p-2 text-left border rounded-lg transition-colors ${watchedValues.toPaymentModeId === paymentMode.id
                              ? 'border-indigo-500 bg-indigo-50'
                              : 'border-gray-200 hover:border-gray-300 hover:bg-gray-50'
                              }`}
                          >
